...
CREATE TABLE IF NOT EXISTS gym_battles (
    ...
    defender_pokemon_id UUID REFERENCES pokemon(id) ON DELETE SET NULL,
...
('灼伤', 'burn', 0, 1.0, NULL, '{"attack": 0.5}', '灼伤状态，每回合损失 1/8 HP，物理攻击降低 50%') ON CONFLICT (name) DO NOTHING,
('麻痹', 'paralyze', 0, 0.75, NULL, '{"speed": 0.5}', '麻痹状态，速度降低 50%，有 25% 概率无法行动') ON CONFLICT (name) DO NOTHING,
('冰冻', 'freeze', 0, 0.2, NULL, '{}', '冰冻状态，有 80% 概率无法行动，被火属性技能攻击后解除') ON CONFLICT (name) DO NOTHING,
('中毒', 'poison', 0, 1.0, NULL, '{}', '中毒状态，每回合损失 1/8 HP') ON CONFLICT (name) DO NOTHING,
('剧毒', 'toxic', 0, 1.0, NULL, '{}', '剧毒状态，每回合损失递增 HP（n/16）') ON CONFLICT (name) DO NOTHING,
('睡眠', 'sleep', 0, 0.0, 2, '{}', '睡眠状态，无法行动 1-3 回合后自动醒来') ON CONFLICT (name) DO NOTHING,
('混乱', 'confusion', 0, 0.67, 3, '{}', '混乱状态，有 33% 概率攻击自己') ON CONFLICT (name) DO NOTHING;
...
-- 更新 gym_pokemon 表添加战斗相关字段
ALTER TABLE gym_pokemon 
