...
ALTER TABLE spawn_statistics ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP DEFAULT NOW();
-- [fix_sql_dialect] 放开旧表中新定义没有的非主键列的 NOT NULL
DO $relax$ DECLARE c RECORD; BEGIN
  FOR c IN SELECT a.attname FROM pg_attribute a
           WHERE a.attrelid = to_regclass('public.spawn_statistics') AND a.attnum > 0 AND NOT a.attisdropped AND a.attnotnull
             AND a.attname NOT IN ('geohash', 'date', 'hour', 'total_spawns', 'spawns_by_rarity', 'captures', 'despawns', 'avg_active_players', 'created_at', 'updated_at', 'id')
             AND NOT EXISTS (SELECT 1 FROM pg_index i WHERE i.indrelid = a.attrelid AND i.indisprimary AND a.attnum = ANY(i.indkey))
  LOOP
...
DROP TRIGGER IF EXISTS update_spawn_cell_configs_updated_at ON spawn_cell_configs;
CREATE TRIGGER update_spawn_cell_configs_updated_at
    BEFORE UPDATE ON spawn_cell_configs
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_spawn_events_updated_at ON spawn_events;
CREATE TRIGGER update_spawn_events_updated_at
    BEFORE UPDATE ON spawn_events
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_spawn_statistics_updated_at ON spawn_statistics;
