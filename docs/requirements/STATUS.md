...
- 当前需求：144/10000
- 剩余需求：9856 条
- 最新生成：REQ-00144（游戏客户端高对比度模式支持系统，无障碍 a11y）
- 最新完成：REQ-00142（location-service spawnConfig 路由挂载与集成，集成与修复）
...
- **优先级**: P1
- **状态**: new
- **影响**: 增强战斗策略深度，支持一键连招，提升游戏趣味性
- **关键特性**:
  - 玩家可为精灵创建最多 5 个技能组合方案
  - 每个组合包含 2-5 个技能步骤
  - 支持设置技能延迟和执行条件（命中后/暴击后/闪避后）
  - 战斗中一键触发预设连招
  - 连招效果系统（伤害加成、状态附加、冷却缩减）
  - 连招统计数据记录
  - 4 张数据库表、8+ 个 API 端点
  - 6 个 Prometheus 指标
