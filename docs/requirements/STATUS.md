...
>
> ⚠️ **创建或实现任何需求前，必须先阅读并遵守 [GUIDELINES.md](./GUIDELINES.md)（需求生成与质量规范）。**
...
||| 稳定性与高可用 | 15 | 10 | 具备限流、降级能力，缺少容灾切换，分布式锁服务已规划（REQ-00105） |
...
- **优先级**: P1
- **状态**: new
- **影响**: 实现 Pokemon GO 风格的精灵恢复站系统，让玩家可以在地图上的特定地点恢复精灵 HP、PP 和状态异常
- **关键特性**:
  - 恢复站分布与发现（城市、公园、商圈等）
  - 精灵恢复机制（HP、PP、状态异常）
  - 恢复站类型与等级（普通、高级、会员、活动）
  - 恢复站互动功能（签到、照片、评论、收藏）
  - 恢复站管理（运营端）
  - 5 张数据库表、10 个 API 端点
  - 5 个 Prometheus 指标
...
- **优先级**: P1
- **状态**: done
- **影响**: 实现完整的游戏内截图分享系统，支持 6 种场景、5 个社交平台
- **关键特性**:
  - 截图捕获引擎（Canvas/WebGL 截图）
  - 6 种分享场景（捕捉、成就、战斗、图鉴、好友、自定义）
  - 5 个社交平台（微信、微博、Twitter、Facebook、系统分享）
  - 水印与玩家信息栏
  - 分享模板系统
  - 分享统计追踪
  - 5 张数据库表、6 个 API 端点
...
- **完成时间**: 2026-06-13 08:15
- **类别**: 国际化/本地化
- **优先级**: P1
- **状态**: done
- **影响**: 为 OpenAPI 文档提供中/英/日三语言描述，降低国际开发者接入成本
- **关键特性**:
  - 多语言描述文件结构（zh-CN/en-US/ja-JP）
  - 语言切换 API 端点（/api-docs/:lang）
  - Swagger UI 语言切换集成
  - 翻译键提取与验证工具
  - 错误码描述国际化
  - CI 自动验证翻译覆盖率
...
- **优先级**: P1
- **状态**: new
- **影响**: 填补反作弊体系最后一环，检测内存修改器、代码注入、协议重放攻击
- **关键特性**:
  - 关键数据完整性校验（CP、IV、货币）
...
- **完成时间**: 2026-06-12 10:10
- **类别**: 集成与修复
- **优先级**: P0
- **状态**: done
- **影响**: 解锁 REQ-00054（道馆战斗系统）的全部功能，7 个战斗 API 端点立即可用
- **关键特性**:
  - 挂载 battle.js 路由到 /api/v1 路径
  - 解锁开始战斗、执行回合、切换精灵等核心功能
  - 解锁战斗回放、队伍预设等辅助功能
  - 路由已在之前提交中挂载，本次审核确认状态
  - 涉及 7 个 API 端点
...
- **完成时间**: 2026-06-12 09:15
- **类别**: 测试覆盖
- **优先级**: P1
- **状态**: done
- **影响**: 消息中心 8 个 API 端点的单元测试覆盖，39 个测试用例全部通过
- **关键特性**:
  - 通知类型验证、格式化函数测试
  - 分页、筛选、状态过滤逻辑测试
  - 批量操作、缓存 TTL 测试
...
- **完成时间**: 2026-06-12 08:15
- **类别**: 集成与修复
- **优先级**: P0
- **状态**: done
- **影响**: 解锁 REQ-00075（IP 黑名单与恶意 IP 自动封禁系统）的用户申诉功能，3 个端点立即可用
- **关键特性**:
  - 挂载 ipAppeal.js 路由到 /ip-appeal 路径
  - 解锁提交封禁申诉、查询申诉状态、检查 IP 状态功能
  - 为被封禁用户提供申诉渠道
  - 无需额外开发，仅挂载即可使用
  - 限流配置：10 次/分钟
...
- **优先级**: P0
- **状态**: new
- **影响**: 解锁游戏活动系统的全部功能，11 个端点立即可用
- **关键特性**:
  - 挂载 events.js 路由到 /events 路径
  - 解锁活动列表、活动详情、参与活动等核心功能
  - 解锁活动进度、奖励领取、排行榜等功能
  - 解锁活动任务、活动商店等功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P0
- **状态**: new
- **影响**: 解锁好友系统的全部功能，17 个端点立即可用
- **关键特性**:
  - 挂载 friends.js 路由到 /friends 路径
  - 解锁好友列表、申请、接受、拒绝等核心功能
  - 解锁在线状态、亲密度查询、好友推荐等功能
  - 解锁拉黑、备注、赠礼等辅助功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P0
- **状态**: new
- **影响**: 解锁精灵图鉴系统的全部功能，15 个端点立即可用
- **关键特性**:
  - 挂载 pokedex.js 路由到 /pokedex 路径
  - 解锁图鉴查询、精灵详情、属性统计等核心功能
  - 解锁搜索、按类型/世代/稀有度查询等功能
  - 解锁图鉴完成度统计、收藏管理等功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P0
- **状态**: new
- **影响**: 解锁 REQ-00047（精灵道具与背包管理系统）的全部功能，12 个端点立即可用
- **关键特性**:
  - 挂载 inventory.js 路由到 /inventory 路径
  - 解锁背包查询、道具使用、丢弃等核心功能
  - 解锁背包整理、分类查询、容量管理等功能
  - 解锁道具合成、兑换等高级功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P0
- **状态**: done
- **影响**: 解锁 REQ-00051（多货币支持与汇率转换系统）的全部功能，10+ 个端点立即可用
- **关键特性**:
  - 挂载 currency.js 路由到 /currency 路径
  - 解锁货币列表、汇率查询、转换计算等核心功能
  - 解锁用户偏好货币设置、汇率历史查询等功能
  - 解锁管理员汇率更新、统计等功能
  - 无需额外开发，仅挂载即可使用
...
- **类别**: 功能增强
- **优先级**: P1
- **影响**: 提升游戏沉浸感，季节专属内容增加玩家留存率 20%+
- **关键特性**:
  - 四季节自动检测与切换
  - 季节专属精灵池与刷新加成
  - 类型加成系统（春季草系+50%，夏季火系+50%等）
  - 季节任务与成就系统
  - 季节商店与限定道具
  - 前端季节视觉效果（樱花/雪花/落叶粒子）
  - 季节总结报告生成
...
1. ~~**推送通知**：缺少 FCM/APNs 推送系统~~ → ✅ REQ-00136 已规划（FCM/APNs 移动推送通知系统）
...
- 当前需求：156/10000
- 剩余需求：9844 条
- 最新生成：REQ-00156（精灵恢复站系统，功能增强）
- 最新完成：REQ-00155（OpenAPI 文档多语言描述与国际化支持，国际化/本地化）
...
- **优先级**: P1
- **状态**: new
- **影响**: 深化背包系统，增加商业化收入，提升玩家体验
- **关键特性**:
  - 金币/宝石双货币购买背包扩容
  - 7 种扩容配置（基础/精灵球/药水/技能机器/进化/特殊）
  - 动态定价与购买次数限制
  - 成就/活动免费赠送机制
  - 管理员赠送接口
  - 2 张数据库表、3 个 API 端点
...
- **优先级**: P1
- **状态**: new
- **影响**: 提升道馆战斗策略深度，实现完整的属性克制机制，让精灵属性收集更有意义
- **关键特性**:
  - 18 种属性完整克制关系矩阵
  - Pokemon GO 风格伤害公式
  - STAB（同属性加成）机制
...
- 当前需求：136/10000
- 剩余需求：9864 条
- 最新生成：REQ-00136（FCM/APNs 移动推送通知系统，功能增强）
- 最新完成：REQ-00131（payment-service 多货币支持路由挂载与集成，集成与修复）
...
- **优先级**: P1
- **状态**: new
- **影响**: 填补"未覆盖高价值缺口"中的推送通知功能，实现完整的移动端推送系统
- **关键特性**:
  - 支持 Firebase Cloud Messaging (FCM) 和 Apple Push Notification Service (APNs) 双平台推送
  - 多场景推送通知（精灵捕捉、道馆战斗、好友请求、活动提醒等）
  - 用户推送偏好管理（全局开关、静默时段、类型订阅）
  - 推送活动管理（创建、调度、发送、统计）
  - 推送统计分析（送达率、打开率、转化率追踪）
  - 6 张数据库表、15+ 个 API 端点
  - 6 个 Prometheus 指标
...
- **优先级**: P0
- **状态**: new
- **影响**: 解锁 REQ-00076（精灵成就系统与里程碑奖励）的全部功能，8 个端点立即可用
- **关键特性**:
  - 挂载 achievements.js 路由到 /achievements 路径
  - 解锁成就列表查询、进度概览等核心功能
  - 解锁成就奖励领取、称号系统等功能
  - 解锁成就排行榜功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P1
- **状态**: new
- **影响**: 为精灵数据提供完整的备份与恢复机制，确保玩家核心资产永不丢失
- **关键特性**:
  - 用户主动创建精灵数据快照备份（最多5个）
  - AES-256-GCM 加密存储
  - 每日/每周自动备份设置
...
- **优先级**: P1
- **状态**: new
- **影响**: 为精灵提供装饰物定制功能，提升个性化和商业化潜力，玩家留存率提升 15%+
- **关键特性**:
  - 6 大装饰物类别（帽子、眼镜、配饰、贴纸、光环、尾迹）
  - 5 种稀有度（普通到传说）
  - 多种获取渠道（商店、成就、活动、合成）
  - 装饰物预设与快速应用
  - PVP/道馆战斗中展示
...
- **创建时间**: 2026-06-11 18:30
- **类别**: 集成与修复
- **优先级**: P0
- **状态**: new
- **影响**: 解锁 REQ-00055（精灵收藏展示系统）的全部功能，11 个端点立即可用
- **关键特性**:
  - 挂载 showcase.js 路由到 /pokemon 路径
  - 解锁收藏管理、点赞、评语、排行榜等核心功能
  - 解锁用户展示页查询功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P1
- **状态**: new
- **影响**: 统一配置管理，配置变更无需重启服务，运维效率提升 50%+
- **关键特性**:
  - 集中管理所有微服务配置（Redis + PostgreSQL）
  - 配置热更新，实时生效
  - 版本控制与一键回滚
  - 多环境支持（dev/staging/prod）
  - 配置变更审计日志
  - 实时推送（EventBus + Redis Pub/Sub）
  - 降级保护（配置中心不可用时使用缓存）
  - 7 个管理 API 端点
  - 5 个 Prometheus 指标
  - 30+ 单元测试
...
- **优先级**: P0
- **状态**: new
- **影响**: 解锁 REQ-00074（玩家排行榜系统）的全部功能，6 个端点立即可用
- **关键特性**:
  - 挂载 leaderboard.js 路由到 /leaderboard 路径
  - 解锁排行榜查询、用户排名、赛季列表等核心功能
  - 解锁赛季奖励领取、历史排名查询等辅助功能
  - 支持 cp/catches/gym_wins/friendship 等多种排行榜类型
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P0
- **状态**: new
- **影响**: 解锁 REQ-00099（游戏消息中心与通知管理系统）的全部功能，8 个端点立即可用
- **关键特性**:
  - 挂载 messageCenter.js 路由到 /notifications 路径
  - 解锁通知列表查询、未读数量统计等核心功能
  - 解锁批量标记已读、删除通知、清除已读等操作
  - 解锁通知统计、偏好设置等辅助功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P0
- **状态**: done
- **影响**: 解锁 REQ-00065（精灵进化与成长系统）的全部功能，7 个端点立即可用
- **关键特性**:
  - 挂载 evolution.js 路由到 /pokemon 路径
  - 解锁进化检查、进化执行、经验值添加等核心功能
  - 解锁进化历史查询、进化道具查询等辅助功能
  - 无需额外开发，仅挂载即可使用
...
- **优先级**: P1
- **状态**: new
- **影响**: 高峰期请求超时率降低 80%+，资源利用率提升 40%+
- **关键特性**:
  - 自适应连接池大小（根据负载动态调整）
  - 查询优先级队列（CRITICAL/HIGH/NORMAL/LOW）
  - 负载感知路由（负载分数计算）
  - 连接预热机制（预测高峰期）
  - 连接健康检查（自动检测移除问题连接）
  - 管理 API（状态查询、配置调整）
