...
> 口径：状态不是 `done` 的需求即视为未完成；同一编号有多个文件时逐个列出并标注"重号"。  
...
| 编号 | 标题 | 涉及服务 | 本轮处置 |
|---|---|---|---|
| [REQ-00040](./REQ-00040-redis-cache-layer.md) (重号) | 实现 Redis 分布式缓存层 | api-gateway, core-service | 本轮：Phase 4（网关/服务 Redis 读缓存修复与接入） |
| [REQ-00041](./REQ-00041-game-client-memory-scanning-protection.md) (重号) | Game Client Dynamic Memory Scanning and Protection System | game-client | 延后：纯客户端内存防护，需原生客户端，服务端无法落地 |
| [REQ-00042](./REQ-00042-distributed-tracing-log-aggregation.md) (重号) | 分布式追踪与日志聚合平台集成 | gateway, user-service, catch-service, pokemon-serv | 本轮：Phase 4（服务端 trace-id 贯通 + 结构化日志） |
| [REQ-00044](./REQ-00044-implement-gdpr-data-compliance.md) (重号) | 实现 GDPR 兼容的数据删除与导出接口 | user-service, data-service | 本轮：Phase 4（GDPR 导出/删除接口挂载可用） |
| [REQ-00558](./REQ-00558-team-realtime-collaboration-voice-chat.md) | 游戏客户端团队实时协作与语音通信系统 | game-client, social-service, gym-service, WebSocke | 延后：实时语音需 WebRTC/TURN 基础设施，超出本轮范围 |
| [REQ-00565](./REQ-00565-database-sensitive-field-encryption-system.md) | 数据库敏感字段透明加密系统 | backend/shared/crypto、user-service、payment-service | 本轮：Phase 4（手机号字段加密 + 盲索引） |
| [REQ-00586](./REQ-00586-gps-location-spoofing-detection-system.md) (重号) | GPS 位置欺骗检测与虚拟定位防护系统 | game-client、gateway、location-service、backend/secur | 本轮：Phase 4（服务端 GPS 速度/瞬移检测） |
| [REQ-00592](./REQ-00592-production-deployment-health-rollback.md) | 生产环境部署健康检查与自动回滚系统 | k8s-operator, cicd-pipeline, monitoring | 本轮：Phase 4（PM2 部署健康检查 + 自动回滚脚本） |
