...
| 验收标准 | 结果 | 说明 |
|---|---|---|
| 精灵详情页面可通过键盘快捷键（V）触发完整语音描述 | ✅ | `V` 键：地图上聚焦精灵卡片或在捕捉页时获取完整语音描述。客户端无独立"精灵详情页"，接口也支持玩家精灵 `/v1/pokemon/my/:id/voice-description` |
| 语音描述包含精灵名称、等级、属性、外观、属性值、技能、进化路径等完整信息 | ✅ | `pokemon-service/src/voiceDescriptionService.js`：名称/图鉴号、属性、稀有度、外观、能力、个体（CP/HP/IV/闪光）、技能、进化路径；野生精灵无等级字段，以 CP 描述 |
| 捕捉场景提供距离音频提示（距离越近音调越高） | ✅ | `mapSpeech.toneForDistance`（0m 880Hz → ≥1000m 220Hz）：进入捕捉、聚焦卡片时播放 |
| 捕捉成功/失败播放不同音效并播报结果 | ✅ | 成功/逃跑/挣脱不同提示音（earcon）+ 结果播报与字幕 |
| 精灵移动时播报方向和距离 | ✅ | 最近精灵方位变化或距离变化 ≥20m 时播报（10 秒节流）；新精灵出现播报方位距离 |
| 支持中/英/日三语语音描述 | ✅ | 生成器与客户端文案 zh-CN / en-US / ja-JP（单测 + 冒烟覆盖三语） |
| 与现有屏幕阅读器兼容（NVDA、JAWS、VoiceOver） | ⚠️ | 真机读屏未验证 |
| 满足 WCAG 2.1 AA 标准（SC 1.1.1 非文本内容、SC 1.3.1 信息和关系） | ✅ | 装饰 emoji aria-hidden、卡片/资源条文字替代、标题与区域语义；axe 无严重问题 |
| 后端 API `/api/pokemon/:id/voice-description` 返回结构化语音描述数据 | ✅ | 实际路径 `GET /v1/pokemon/species/:id/voice-description?lang=&cp=`（网关 /v1/pokemon/* 鉴权，与现有 /v1 约定一致，非 /api/pokemon） |
| 批量 API 支持列表场景的语音描述获取 | ✅ | `GET /v1/pokemon/voice-descriptions?ids=1,4,7&lang=`（≤50） |

- 入口：`frontend/game-client/index.html` → `src/bootstrap/features.js` → `src/bootstrap/a11y.js` 的 `initAccessibility(ctx)`；设置入口「我的 → 无障碍设置」（`window.showAccessibilitySettings`，或按 `,`），模块在 `src/accessibility/`；后端：pokemon-service `src/routes/voiceDescription.js`（挂载于 `/pokemon`），经网关 `/v1/pokemon/*`（已有鉴权代理，未改网关）
- 迁移：无（使用已有 pokemon_species / pokemon_instances）
...
- 待验证：NVDA/JAWS/VoiceOver 下 V 键描述朗读；三语描述文案自然度；硬件相关（振动/手柄/Web Speech）以模拟对象测试，⚠️ 真机未验证
