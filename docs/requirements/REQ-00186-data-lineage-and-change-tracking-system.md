...
```sql
-- database/pending/20260614_070000__add_data_lineage_system.sql
...
-- 血统统计分析视图
CREATE MATERIALIZED VIEW IF NOT EXISTS lineage_statistics AS
SELECT 
  source_entity_type,
  operation,
  COUNT(*) as operation_count,
  COUNT(DISTINCT user_id) as unique_users,
  AVG(EXTRACT(EPOCH FROM (created_at - LAG(created_at) OVER (PARTITION BY source_entity_type ORDER BY created_at)))) as avg_interval_seconds
FROM data_lineage
GROUP BY source_entity_type, operation;

CREATE UNIQUE INDEX idx_lineage_stats_pk ON lineage_statistics(source_entity_type, operation);
```
...
class DataLineageTracker {
   ...
   * @param {string} params.targetType - 目标实体类型
   * @param {string} params.targetId - 目标实体ID
   * @param {string} params.sourceType - 源实体类型（可选，如孵化来源）
   * @param {string} params.sourceId - 源实体ID（可选）
   * @param {string} params.operation - 操作类型
   * @param {string} params.operationId - 业务操作ID
   * @param {Object} params.afterSnapshot - 创建后快照
  ...
  async recordSplit(params) {
    return this._recordLineage({
      lineageType: LINEAGE_TYPES.SPLIT,
      ...params
    });
  }
   ...
   * 记录实体合并血缘（如道具合并）
   */
  async recordMerge(params) {
    return this._recordLineage({
      lineageType: LINEAGE_TYPES.MERGE,
      ...params
    });
  }
  ...
  async _recordLineage(params) {
    ...
    try {
      ...
      // 使影响范围缓存失效
      await this._invalidateImpactCache(targetType, targetId);
      if (sourceId) {
        await this._invalidateImpactCache(sourceType, sourceId);
      }

      span.setAttributes({
        'lineage.id': result.rows[0].id,
      ...
      logger.error('Failed to record data lineage', { error: err.message, params });
      throw err;
    } finally {
      span.end();
    }
   ...
   * 恢复实体到指定版本（基于血缘快照）
   */
  async restoreToVersion(entityType, entityId, lineageId) {
    const lineageResult = await this.db.query(`
      SELECT before_snapshot, after_snapshot, lineage_type
      FROM data_lineage
      WHERE id = $1 AND target_entity_type = $2 AND target_entity_id = $3
    ...
    const { before_snapshot, lineage_type } = lineageResult.rows[0];

    if (lineage_type === LINEAGE_TYPES.DELETION) {
      // 删除操作，恢复使用 before_snapshot
      return before_snapshot;
    } else if (lineage_type === LINEAGE_TYPES.MUTATION) {
      // 变更操作，恢复使用 before_snapshot
      return before_snapshot;
    }

    throw new Error('Cannot restore from this lineage type');
  }

  /**
   * 使影响范围缓存失效
   */
  async _invalidateImpactCache(entityType, entityId) {
    await this.db.query(`
...
// social-service 交易血缘追踪
async function tradePokemon(fromUserId, toUserId, pokemonId) {
  const lineageTracker = getLineageTracker(db);
  ...
  // 执行转移
  await db.query(`
    UPDATE user_pokemon SET user_id = $1 WHERE id = $2
  `, [toUserId, pokemonId]);

  // 记录转移血缘
  await lineageTracker.recordTransfer({
    sourceType: 'user',
    sourceId: String(fromUserId),
    targetType: 'user',
    targetId: String(toUserId),
    operation: 'trade',
    operationId: tradeId,
    beforeSnapshot,
    afterSnapshot: { ...beforeSnapshot, user_id: toUserId },
    deltaFields: {
      user_id: { from: fromUserId, to: toUserId }
    },
    userId: fromUserId,
...
const { getLineageTracker } = require('../../shared/dataLineage');
...
/**
 * GET /api/admin/lineage/downstream/:entityType/:entityId
 * 查询实体下游影响范围
 */
router.get('/downstream/:entityType/:entityId', requireAdmin, async (req, res) => {
  const { entityType, entityId } = req.params;
  const { depth = 10 } = req.query;
  
  const lineageTracker = getLineageTracker(db);
  const result = await lineageTracker.traceDownstream(entityType, entityId, parseInt(depth));
...
/**
 * GET /api/admin/lineage/history/:entityType/:entityId
 * 获取实体变更历史
 */
router.get('/history/:entityType/:entityId', requireAdmin, async (req, res) => {
  const { entityType, entityId } = req.params;
  const { limit = 50, offset = 0, operation } = req.query;
  
  const lineageTracker = getLineageTracker(db);
  const history = await lineageTracker.getChangeHistory(
    entityType, entityId, 
...
const lineageRecordsTotal = new Counter({
  name: 'minego_lineage_records_total',
  help: 'Total number of lineage records created',
  labelNames: ['service', 'lineage_type', 'operation']
});

const lineageQueryDuration = new Histogram({
  name: 'minego_lineage_query_duration_seconds',
  help: 'Duration of lineage queries',
  labelNames: ['query_type'],  // upstream, downstream, history, graph
  buckets: [0.01, 0.05, 0.1, 0.5, 1, 2, 5]
});

const lineageDepth = new Histogram({
  name: 'minego_lineage_trace_depth',
  help: 'Depth of lineage traces',
  labelNames: ['direction'],  // upstream, downstream
