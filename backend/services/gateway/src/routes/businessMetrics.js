...
const { asyncHandler } = require('../../../../shared/errorHandler');
 ...
 * 获取实时业务指标
 */
router.get('/realtime', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  if (!metricsCollector) {
    throw Errors.serviceUnavailable({ service: 'metrics-collector' }, { reason: 'not_initialized' });
  }
 ...
 * 获取小时级指标数据
 */
router.get('/hourly', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  if (!metricsCollector) {
    throw Errors.serviceUnavailable({ service: 'metrics-collector' }, { reason: 'not_initialized' });
  }
 ...
 * 获取日级指标数据
 */
router.get('/daily', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  if (!metricsCollector) {
    throw Errors.serviceUnavailable({ service: 'metrics-collector' }, { reason: 'not_initialized' });
  }
 ...
 * 获取地理分布数据
 */
router.get('/geo', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  if (!metricsCollector) {
    throw Errors.serviceUnavailable({ service: 'metrics-collector' }, { reason: 'not_initialized' });
  }
...
router.post('/event', authenticate, asyncHandler(async (req, res) => {
  ...
  const validEventTypes = [
    'player_online', 'player_offline', 'pokemon_catch', 'pokemon_spawn',
    'pokemon_evolve', 'pokemon_trade', 'gym_battle', 'raid',
    'friendship', 'gift', 'message', 'payment', 'refund'
  ];

  if (!validEventTypes.includes(eventType)) {
    throw Errors.invalidRequest({ eventType }, { validTypes: validEventTypes });
  }
 ...
 * 获取业务指标摘要（适用于仪表板概览）
 */
router.get('/summary', authenticate, requireAdmin, asyncHandler(async (req, res) => {
  if (!metricsCollector) {
    throw Errors.serviceUnavailable({ service: 'metrics-collector' }, { reason: 'not_initialized' });
  }
